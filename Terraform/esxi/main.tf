...
resource "esxi_virtual_disk" "longhorn_plex" {
  virtual_disk_disk_store = var.disk_store
  virtual_disk_dir        = "k3s-cilium-plex"
  virtual_disk_name       = "longhorn.vmdk"
  virtual_disk_size       = 200
  virtual_disk_type       = "thin"
}
...
resource "esxi_guest" "k3s-cilium-plex" {
  guest_name             = "k3s-cilium-plex"
  guest_shutdown_timeout = 20
  guest_startup_timeout  = 120
  ...
  # - Longhorn
  virtual_disks {
    virtual_disk_id = esxi_virtual_disk.longhorn_plex.id
    slot            = "0:1"
  }
